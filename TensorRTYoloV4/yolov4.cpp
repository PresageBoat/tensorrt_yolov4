...
int YoloV4Detection::RunInference( float* inputdata,float* outputprob,const int batchsize) {

	//��batch��Ϊ1ʱ��batchsize ���Ƽ���СΪ4��������
	if (batchsize!=1)
	{
		if (batchsize&3!=0)
		{
	...
	return doInference(*context, inputdata, outputprob, batchsize);;
}
...
//}
//
//cv::Rect YoloV4Detection::get_rect(cv::Mat& img, float bbox[4]) {
//	int l, r, t, b;
//	float r_w = INPUT_W / (img.cols * 1.0);
//	float r_h = INPUT_H / (img.rows * 1.0);
//	if (r_h > r_w) {
